...
function DriverDetailInner() {
  ...
  const handleAction = async (actionId: string, cost: number, statImpact: Record<string, number>, label: string) => {
    if (!player || player.credits < cost) { alert("Not enough credits!"); return; }
    setPerformingAction(actionId);
    try {
      await updateDoc(doc(db, "players", user!.uid), { credits: increment(-cost) });
      const updates: any = {};
      Object.entries(statImpact).forEach(([stat, val]) => { updates[`stats.${stat}`] = increment(val); });
          ...
          <div className="space-y-6">
            <div className="card p-6 overflow-hidden relative">
              ...
              <div className="flex items-center gap-4 mb-6 relative">
                ...
                <div>
                  <h1 className="text-2xl font-black" style={{ fontFamily: "var(--font-display)" }}>{driver.name}</h1>
                  <div className="flex items-center gap-2 text-sm text-[var(--color-text-secondary)] font-mono">
                    ...
                    <span>•</span><span>Level {driver.level || 1}</span>
                    <span>•</span><span>Age {driver.age}</span>
                  </div>
                </div>
              </div>
                  ...
                  return (
                    <div key={key}>
                      ...
                      <div className="h-1.5 bg-[var(--color-border)] rounded-full overflow-hidden">
                        <motion.div initial={{ width: 0 }} animate={{ width: `${val}%` }} className="h-full rounded-full" style={{ backgroundColor: config.color }} />
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {activities.map((act) => (
                  <button key={act.id}
                    ...
                    className={`card p-5 text-left transition-all relative group ${(player?.credits || 0) < act.cost ? "opacity-50 grayscale" : "hover:border-[var(--color-text-muted)]"}`}
                  >
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="text-sm font-bold font-mono text-[var(--color-racing-green)]">${act.cost.toLocaleString()}</div>
                    </div>
                    <h3 className="font-bold mb-1">{act.label}</h3>
                    <p className="text-xs text-[var(--color-text-secondary)] leading-relaxed mb-4">{act.desc}</p>
                    <div className="flex items-center gap-2">
